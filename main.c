...
void displayVerticalChart() {
    int maxBarHeight = 10; // Set the vertical chart to be max 10 rows tall
    int maxValue = getMaxValue();
    ...
    int categorySpace = MAX_NAME_LEN + 2; // space for bar and space

    // Center the chart by calculating left padding
    int chartPadding = (CHART_WIDTH - num_categories * categorySpace) / 2;
    chartPadding = chartPadding < 0 ? 0 : chartPadding;

    // Print the vertical bars
    for (int row = maxBarHeight; row >= 0; --row) {
        printf("%*s", chartPadding, ""); // Left padding for centering the chart

        // Print bars and categories side by side
        for (int j = 0; j < num_categories; ++j) {
            int barHeight = (int)(categories[j].value / scale);
            printf("%c%-*s", barHeight >= row ? '#' : ' ', MAX_NAME_LEN + 1, ""); // Print bar for each category
        }

        // Print Y-axis ticks and labels after the bars
        if (row == maxBarHeight) {
    ...
    // Print category names below the chart
    printf("%*s", chartPadding, ""); // Left padding for centering the chart
    for (int i = 0; i < num_categories; ++i) {
        printf("%-*s", MAX_NAME_LEN + 1, categories[i].name);
    }
    printf("\n");

    // Print category values below the chart
    printf("%*s", chartPadding, ""); // Left padding for centering the chart
    for (int i = 0; i < num_categories; ++i) {
        printf("%-*d", MAX_NAME_LEN + 1, categories[i].value);
    }
    printf("\n");
}
